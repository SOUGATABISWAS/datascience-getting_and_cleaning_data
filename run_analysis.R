...
tmpTrain <- read.table("X_train.txt")
## Extract Test Data to tmpTest
tmpTest <- read.table("X_test.txt")
## Merge Testing and Training data into a single dataTable.
X <-rbind(tmpTrain,tmpTest)

## Extract Subject Train to Data 
tmpSubjectTrain <- read.table("subject_train.txt")
## Extract Subject Train to Data 
tmpSubjectTest <- read.table("subject_test.txt")
subject <-rbind(tmpSubjectTrain,tmpSubjectTest)

## Extract Y Training Data 
tmpYTrain <- read.table("y_train.txt")
## Extract Subject Train to Data 
tmpYTest <- read.table("y_test.txt")
